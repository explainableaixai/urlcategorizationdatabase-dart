# URL Categorization Database for Dart and Flutter

`urlcategorizationdatabase` is the Dart and Flutter tiered URL content-classification client for [URL Categorization Database](https://www.urlcategorizationdatabase.com). It gives applications a small, typed interface for a production data service while keeping authentication, URL construction, response decoding, retries, and error handling out of business logic.

Applications that make decisions about individual URLs need a stable content taxonomy with both broad labels and useful subcategories instead of an opaque one-word guess. The underlying database covers more than 120 million domains and supports IAB-style top-level classification as well as hundreds of more specific tier-two categories.

This package is designed as a real client library rather than a collection of copied HTTP examples. It supports the normal lifecycle of a lookup: validate input, send the API key in the expected header, apply a bounded timeout, decode a successful response, distinguish authentication and quota failures, and expose response fields without discarding information that may be needed later. It fits contextual targeting, content routing, portfolio analysis, category-aware enrichment and other applications where the decision must be repeatable and auditable.
...
| `tier` | Preserved from the service response for typed access, logging, or policy decisions. |
...
Use the implementation that matches the deployment environment. These repositories and registry pages all focus on URL Categorization Database rather than unrelated products:

- [JavaScript repository](https://github.com/explainableaixai/urlcategorizationdatabase)
- [Composer repository](https://github.com/optimiser4/urlcategorizationdatabase)
- [npm client](https://www.npmjs.com/package/urlcategorizationdatabase)
- [Rust client](https://crates.io/crates/urlcategorizationdatabase)
- [PHP client](https://packagist.org/packages/urlcategorizationdatabase/urlcategorizationdatabase)
...
- [AI Tools Blocklist categorization behind this Dart and Flutter URL Categorization Database client](https://www.aitoolsblocklist.com)
- [Dart and Flutter workflow for exposing unapproved AI adoption alongside URL Categorization Database](https://www.shadowaitools.com)
- [Dart and Flutter companion data for acceptable-use reporting within URL Categorization Database deployments](https://www.webfilteringdatabase.com)
- [Dart and Flutter enrichment layer connecting URL Categorization Database with content enrichment](https://www.websitecategorizationapi.com)
